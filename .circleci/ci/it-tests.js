...
const updateGraphqlClientConfiguration = (pid) => {
    ...
    ci.sh(`curl -v "http://localhost:5502/system/console/configMgr/${pid}" \
                -u "admin:admin" \
                -d "apply=true" \
...
const updateGraphqlProxyServlet = () => {
    ci.sh(`curl -v "http://localhost:5502/system/console/configMgr/com.adobe.cq.cif.proxy.GraphQLProxyServlet" \
                -u "admin:admin" \
                -d "apply=true" \
...
try {
    ...
    ci.dir(qpPath, () => {
        ...
        // Start CQ
        ci.sh(`./qp.sh -v start --id author --runmode author --port 5502 --qs-jar /home/circleci/cq/author/cq-quickstart.jar \
